...
import jwt_decode from "jwt-decode"; // giai ma ma hoa tai khoan user
...
export const TokenUserInitiate = () => {
  return async function (dispatch) {
    ...
    try {
      const user = await axios.post("http://localhost:5000/token");
      dispatch(TokenUserSuccess(user));
    } catch (error) {
      if (error.response) {
        dispatch(TokenUserFail(error.response.data.msg));
      }
    }
  };
};
